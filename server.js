...
app.post('/api/crops/add', (req, res) => {
  try {
    ...
    const newCrop = {
      ...
      auctionEndTime: Date.now() + 120000,  // 2 minutes in milliseconds
      status: 'active',
      bids: [],
...
app.get('/api/crops', (req, res) => {
  try {
    ...
    crops = crops.map(crop => {
      if (crop.status === 'active' && crop.auctionEndTime <= now) {
        crop.status = 'closed';
      }
      return crop;
    });
...
app.post('/api/crops/bid', (req, res) => {
  try {
    ...
    // ✅ auctionEndTime timestamp check
    if (crop.auctionEndTime <= Date.now()) {
      crop.status = 'closed';
      fs.writeFileSync(CROPS_FILE, JSON.stringify(crops, null, 2));
